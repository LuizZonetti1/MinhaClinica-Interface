...
import {
  ...
  updateReceptionProfilePassword,
...
const ReceptionEditProfilePage = () => {
  ...
  const applyLatestProfile = (
    latestProfile: ReceptionProfileData,
    fallbackName: string,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (Object.keys(payload).length > 0) {
        await updateReceptionProfile(payload);
      }
      ...
      if (isChangingPassword) {
        ...
        await updateReceptionProfilePassword(passwordPayload);
      }

      const latestProfile = await getReceptionProfile();
      applyLatestProfile(latestProfile, trimmedName, form.phone);

      notifySuccess("Perfil atualizado com sucesso.");
      navigate("/recepcao/perfil");
    } catch (error: unknown) {
...
export default ReceptionEditProfilePage;
